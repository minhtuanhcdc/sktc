            ...
            <div class=""> 
                <div class="flex items-center space-x-2">
                    <div class="flex items-center w-[25%]">
                        ...
                        <input v-model="form.name" id="phone" type="text" class="w-[80%] border border-blue-700 h-7 rounded-md"  autocomplete="name" />
                    ...
                    <div class="flex items-center w-[15%]">
                        ...
                        <input v-model="form.birth" id="phone" type="date" class="inputText border border-blue-700 w-full h-7 rounded-md"  autocomplete="birth" />
                    </div> 
                    <div class="flex items-center w-[10%] space-x-2">
                        <span class=" text-blue-900 font-bold w-[15%]">GT:</span>
                         <div class="space-x-1 w-[90%]">
                            <input type="radio" v-model="gt" value="nam"/><span class="text-xs">Nam</span>
                            <input type="radio" v-model="gt" value="nu"/><span class="text-xs">Nữ</span>
                        </div>
                    </div>  
                    <div class="flex items-center w-[20%]">
                        <label class="w-[20%] text-blue-900 font-bold text-sm">Tên mẹ:</label>
                        <input v-model="form.mst" id="mst" type="text" class="w-[80%] border border-blue-700 flex-1 h-7 rounded-md"  autocomplete="mst" />
                    </div> 
                    <div class="flex items-center w-[15%]">
                        <label class="w-[30%] leading-4 text-blue-900 font-bold text-sm flex justify-end">Mã Đd:</label>
                        <input v-model="form.sohieu" id="sohieu" type="text" class="w-[70%] border border-blue-700 h-7 rounded-md"  autocomplete="sohieu" />
                    </div>  
                    <div class="flex items-center w-[15%]">
                        <label class="w-[25%] leading-4 text-blue-900 font-bold text-sm">Mã TC:</label>
                        <input v-model="form.sohieu" id="sohieu" type="text" class="w-[75%] border border-blue-700 h-7 rounded-md" autocomplete="sohieu" />
                    </div>  
                </div>
                ...
                <div class="flex mt-4 space-x-4">
                    <div class="w-1/2 flex space-x-4">
                        <div class="flex items-center">
                            ...
                            <input class="h-7 rounded-sm border w-14"/>
                        </div>
                    ...
                    <div class="w-1/2 border border-hcdc1 p-2 flex">
                            <div class="w-[25%] flex" v-if="month_birth<=24">
                                <span class="w-[60%] h-7 font-bold">Nhap SKĐK: </span>
                                <input class="w-[40%] h-7 rounded-sm"/>
                            </div>
                            <div v-if="month_birth>=6 &&  month_birth<=36">
                                <span class="h-7 font-bold">Vitamin: </span>
                                <input class="h-7 rounded-sm"/>
                            </div>
                    </div>
                </div>
                <div class=" mt-4">
                    <fieldset class="border border-solid border-blue-900 px-2 bg-gray-100 h-auto overflow-y-auto">
                        ...
                        <div class="flex space-x-2 px-0 items-center">
                            ...
                            <div class="w-[20%]">
                                ...
                                <input type="date" class="h-7"/>
                            </div>
                            <div class="w-[10%] flex items-center ">
                                <label class="w-[60%] font-bold leading-3">Cân nặng(kg):</label>
                                <input type="text" class="w-[40%] h-7 p-0 text-hcdc2"/>
                            </div>
                            <div class="w-[10%] flex items-center ">
                                <label class="w-[60%] font-bold leading-3">Chiều cao(cm):</label>
                                <input type="text" class="w-[40%] h-7 p-0 text-hcdc2"/>
                            </div>
                        </div>
                    </fieldset>
...
import InputErrorApp from '../../Components/InputError.vue'
    export default{
        ...
        data(){
            return{
                ...
                form: this.$inertia.form({
                  ...
                  id_district:'',
                  id_ward:'',
                  usd_exchange:this.currentExchangeFix?this.currentExchangeFix:this.cutExchangeOnline(this.currencyVietcomBank),
                  editChangeService:'',  
                 sohieu:'' , 
                 test:'sadfd' , 
              },
        ...
        methods:{
            cutExchangeOnline(data){
                //console.log('Kiểu:.....',typeof data)
                return data.substring(-1,6).replace(',','');
            }, 
            reset(){
                this.edit=false
                this.form.sohieu = ''
            ...
            saveCustommerEmit(e){
                ...
                var data=[this.form,this.getExchange];
                this.$emit('saveCustommerEvent',data)
            },
            formatPrice_1(value) {
