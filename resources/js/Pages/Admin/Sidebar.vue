    ...
    <div class="flex flex-col  p-0 bg-hcdc1 z-40 mt-2" :class="isOpenMenu?'w-56':'w-14'">
        ...
        <div class="flex w-full p-2 z-40">
            <div class="flex flex-col  w-8 space-y-4">
                <span class="text-right flex justify-end items-center h-10" v-for="(menu,i) in $page.props.menuPermission.menuAccess" :key="i">
                        <NavLink v-if="isOpenMenu"  class="text-white capitalize border-t w-full h-10" :class="menu.menu_group==2?'bg-green-600':''"  >
                            <span class=""> 
                                <component v-bind:is="menu.icon"  class="w-6 h-6" :class="route().current(menu.Url)?'text-hcdc2':'text-white'"/>
                            </span>
                        </NavLink>
                        <NavLink v-else :href="route(menu.Url)" class="text-white capitalize border-t w-full h-10" :class="menu.menu_group==2?'bg-green-600':''"  >
                            <span class="tooltip_menu z-50 text-xs" :data-tip="menu.menuName"> 
                                <component v-bind:is="menu.icon"  class="w-6 h-6" :class="route().current(menu.Url)?'text-hcdc2':'text-white'"/>
                            </span>
                        </NavLink>
                </span>
