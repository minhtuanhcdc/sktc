    ...
    <AdminLayout>
        <div class="h-[75%]">
          <div class="flex justify-between py-0 px-4">
            <span>Danh sách Bưu Cục(Post)</span>
            <form @submit.prevent="uploadFile">
              <div class="flex flex-row">
                  ...
                  <div class="flex items-center mt-0">
                    <button class="px-2 py-1 text-white bg-blue-900  rounded">Upload Bưu cục</button>
                  </div>
              </div>
          </form>
                    ...
                    <Table :classTable="classTable" :classThead="classThead">
                        ...
                        <template #tbody>
                            ...
                            <TableRow :classRow="classRow"  v-for="(p,i) in posts.data" :key="i">
                                ...
                                <Tbody class="text-center">{{ p.address }}</Tbody>
                                <Tbody class="text-center">{{ p.phone }}</Tbody>
                                <Tbody>{{ formattedDate(p.updated_at) }}</Tbody>
                                <Tbody>{{ p.province_code }}</Tbody>
                                <Tbody></Tbody>
                                <Tbody class="flex justify-end pr-4 space-x-8 z-10"> 
                                    <span class="tooltip_edit z-50 cursor-pointer" data-tip="Sửa" v-if="$page.props.can?$page.props.can.edit:''">
                                        <PencilIcon class="classPencil" @click="openEditPost(p)" />
                                    </span> 
                                    <span title="Xóa">
                                        <XCircleIcon class="classXIcon" @click="openConfirm(p)" /> 
                                    </span>
                                </Tbody>
                            </TableRow>
                            </template>
        ...
        <ModalApp :show="openModal" :maxWidth="maxWidth">
            ...
            <div class="px-6 py-4">
                <form @submit.prevent="savePost">
                    <div class="">
                         <label for="code" class="classLabel">Mã bưu cục</label>
                ...
                <!--Name--->
                    <div class="">
                         <label for="name" class="classLabel">Tên bưu cục(cơ sở)</label>
